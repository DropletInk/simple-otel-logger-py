...
class span_info(TypedDict):
    trace_id:str
    span_id:str
    trace_flags:int

class event_dict(TypedDict):
    span:span_info| None

def add_open_telemetry_spans(logger:object, method_name:str, event_dict:event_dict)->event_dict:
...
def otel_tags(logger:object, method_name:str, event_dict:event_dict)-> event_dict:

    event_dict["instrumentationScope"] = {"name": "simple-otel-logger", "version": "1.0.0"}
    return event_dict


def log_organiser(logger:object, method_name:str, event_dict:event_dict) ->dict:
    return {
...
def rename_level(logger:object, method_name:str, event_dict:event_dict) ->event_dict:
    level_mapping = {
        "DEBUG": 5,
        "INFO": 9,
        "WARNING": 13,
    ...
    if "level" in event_dict:
        ...
        event_dict["severityNumber"] = level_mapping.get(event_dict["severityText"], 0)
    return event_dict
...
class Logger:
    ...
    def __init__(self, service_name=packages_distributions()["pylog"][0]):
        ...
        instrumentationScope = {"name": "simple-otel-logger", "version": "1.0.0"}
        self.logger = structlog.get_logger().bind(resources=resources, instrumentationScope=instrumentationScope)
